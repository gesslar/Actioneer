...
Loops until a predicate returns `false`. The predicate is evaluated **after** each iteration:

```js
import { ActionBuilder, ACTIVITY } from "@gesslar/actioneer"

class ProcessorAction {
  #hasMoreWork = (ctx) => ctx.queue.length > 0
  ...
  setup(builder) {
    builder
      ...
      .do("process", ACTIVITY.UNTIL, this.#hasMoreWork, this.#processItem)
      .do("finish", ctx => { return ctx.processed })
  }
}
```

The activity executes at least once, then continues while the predicate returns `true`. Once it returns `false`, execution moves to the next activity.
...
| **UNTIL**   | `.do(name, ACTIVITY.UNTIL, predicate, operation)`          | After iteration  | Loop until condition is false        |
